...
def run_multihead_self_attention(
    ...
    multihead = MultiHeadSelfAttention(d_model, num_heads, apply_rope=False)
    multihead.qkv_proj.weight.data = torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0)
...
def run_multihead_self_attention_with_rope(
    ...
    """
    from cs336_basics.transformer import MultiHeadSelfAttention
    multihead = MultiHeadSelfAttention(d_model, num_heads, apply_rope=True, max_seq_len=max_seq_len, theta=theta)
    multihead.qkv_proj.weight.data = torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0)
    multihead.output_proj.weight.data = o_proj_weight
...
def run_transformer_block(
    ...
    transformer_block.mha.qkv_proj.weight.data = torch.cat([weights["attn.q_proj.weight"], weights["attn.k_proj.weight"], weights["attn.v_proj.weight"]], dim=0)
    transformer_block.mha.output_proj.weight.data = weights["attn.output_proj.weight"]
...
def run_transformer_lm(
    ...
    transformer = TransformerLM(d_model, num_heads, d_ff, vocab_size, context_length, num_layers, theta=rope_theta)
...
def run_get_lr_cosine_schedule(
    ...
    """
    from cs336_basics.training import get_cosine_lr
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    return train_bpe(str(input_path), int(vocab_size), list(special_tokens))
