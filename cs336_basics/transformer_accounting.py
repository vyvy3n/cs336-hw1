...
def analyze_model_scaling(configs: list[tuple[str, ModelConfig]]) -> dict[str, dict[str, float]]:
    ...
    for name, config in configs:
        ...
        proportions = {
            ...
            "attention": (flops["per_layer"]["attention_qkv"] + 
                         flops["per_layer"]["attention_scores"] + 
                         flops["per_layer"]["attention_weighted_sum"] + 
                         flops["per_layer"]["attention_output"]) * config.num_layers / total_flops,
            "feedforward": (flops["per_layer"]["feedforward_w1"] + 
                           flops["per_layer"]["feedforward_w2"] + 
                           flops["per_layer"]["feedforward_w3"]) * config.num_layers / total_flops,
...
def solve_transformer_accounting():
    ...
    total_flops = flops['total']
    attention_flops = (flops['per_layer']['attention_qkv'] + 
                      flops['per_layer']['attention_scores'] + 
                      flops['per_layer']['attention_weighted_sum'] + 
                      flops['per_layer']['attention_output']) * gpt2_xl.num_layers
    
    feedforward_flops = (flops['per_layer']['feedforward_w1'] + 
                        flops['per_layer']['feedforward_w2'] + 
                        flops['per_layer']['feedforward_w3']) * gpt2_xl.num_layers
    ...
    for name, results in analysis.items():
        print(f"{name:<15} {results['total_flops']:<15,.0f} {results['attention']*100:<12.1f} "
    ...
    print(f"Context length increase: {gpt2_xl.context_length} -> {gpt2_xl_long.context_length} "
          f"({gpt2_xl_long.context_length // gpt2_xl.context_length}x)")
    print(f"Total FLOP increase: {flop_ratio:.1f}x")
    
    attention_ratio = (flops_long['per_layer']['attention_scores'] + 
    ...
    attention_flops_long = (flops_long['per_layer']['attention_qkv'] + 
                           flops_long['per_layer']['attention_scores'] + 
                           flops_long['per_layer']['attention_weighted_sum'] + 
                           flops_long['per_layer']['attention_output']) * gpt2_xl_long.num_layers
    
    feedforward_flops_long = (flops_long['per_layer']['feedforward_w1'] + 
                             flops_long['per_layer']['feedforward_w2'] + 
                             flops_long['per_layer']['feedforward_w3']) * gpt2_xl_long.num_layers
    
    print(f"\nProportion changes with longer context:")
    print(f"Attention: {attention_flops/total_flops*100:.1f}% -> {attention_flops_long/total_flops_long*100:.1f}%")
    print(f"Feed-forward: {feedforward_flops/total_flops*100:.1f}% -> {feedforward_flops_long/total_flops_long*100:.1f}%")
